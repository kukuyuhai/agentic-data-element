...
本项目预置了 6 个跨岗位标准工作流，用于串联多个数字员工完成典型场景。
...
**参与角色**：DAM-04 → DAT-02 → DAT-01 → DAT-03
**触达法规**：数据出境安全评估办法、个人信息出境标准合同办法

```
DAM-04 数据出境安全评估
  ↓
DAT-02 跨境交易合规审查（KYC/KYB、合同审查）
  ↓
DAT-01 跨境交易方案设计
  ↓
DAT-03 跨境交易执行与监控
...
**id**：`data-asset-strategy-planning`
**参与角色**：DAM-01 → DAM-05 → DAM-02 → DAM-06
...
**id**：`data-sharing-open`
**参与角色**：DAM-05 → DAM-04 → DAM-06 → DAT-03
